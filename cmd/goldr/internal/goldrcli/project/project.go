...
func GenerateFilesForPaths(paths Paths) ([]GeneratedFile, error) {
	tree, err := routing.ScanWithMounts(paths.RoutesDir, paths.MountsDir)
	if err != nil {
		return nil, err
	}
...
func PathsForResolvedRoot(ctx context.Context, appRoot string) (Paths, error) {
	...
	return Paths{
		Root:            appRoot,
		RoutesDir:       routesDir,
		MountsDir:       appfs.MountsDir(appRoot),
...
func GenerateManifestFiles(paths Paths, manifest routing.Manifest) ([]GeneratedFile, error) {
	...
	routePackageFiles, err := GenerateRoutePackageFiles(paths, manifest)
	if err != nil {
		return nil, err
	}
	mountURLFiles, err := GenerateMountURLHelperFiles(paths, manifest)
	if err != nil {
		return nil, err
	}
	...
	files = append(files, routePackageFiles...)
	files = append(files, mountURLFiles...)
	return files, nil
}
...
func GenerateRoutePackageFiles(paths Paths, manifest routing.Manifest) ([]GeneratedFile, error) {
	files, err := wiring.GenerateRoutePackageFiles(manifest, wiring.GenerateOptions{
		RouteRootImportPath: paths.RouteImportPath,
		InspectorImportPath: InspectorImportPath(paths),
	})
	if err != nil {
...
func GenerateMountURLHelperFiles(paths Paths, manifest routing.Manifest) ([]GeneratedFile, error) {
	mountPaths := mountedURLHelperMountPaths(manifest)
	files := make([]GeneratedFile, 0, len(mountPaths))
	for _, mountPath := range mountPaths {
		mountDir := filepath.Join(paths.MountsDir, filepath.FromSlash(mountPath))
		packageName, err := packageNameForDir(mountDir)
		if err != nil {
			return nil, err
		}
		source, err := wiring.GenerateMountURLHelpers(manifest, wiring.GenerateMountURLOptions{
		...
		files = append(files, GeneratedFile{
			Path:    filepath.Join(mountDir, wiring.URLGeneratedFileName),
			Content: source,
		})
	}
	return files, nil
...
func mountedURLHelperMountPaths(manifest routing.Manifest) []string {
	seen := make(map[string]bool)
	for _, route := range manifest.Routes {
		if route.Mount == nil || route.Mount.Path == "" {
			continue
		}
		seen[route.Mount.Path] = true
	}
	paths := make([]string, 0, len(seen))
	for mountPath := range seen {
...
func packageNameForDir(dir string) (string, error) {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() ||
			...
			name == wiring.URLGeneratedFileName {
			continue
		}
...
func StaleManagedGeneratedFiles(paths Paths, files []GeneratedFile) ([]string, error) {
	...
	var stale []string
	for _, root := range []string{paths.RoutesDir, paths.MountsDir} {
		found, err := staleManagedGeneratedFiles(root, expected)
		if err != nil {
			return nil, err
		}
...
func staleManagedGeneratedFiles(root string, expected map[string]bool) ([]string, error) {
