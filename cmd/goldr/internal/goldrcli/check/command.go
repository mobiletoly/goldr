...
import (
	...
	"github.com/mobiletoly/goldr/cmd/goldr/internal/goldrcli/scandiag"
...
func checkGeneratedManifestReadiness(paths project.Paths, manifest routing.Manifest) ([]project.GeneratedFile, error) {
	...
	routePackageFiles, err := project.GenerateRoutePackageFiles(paths, manifest)
	if err != nil {
		return nil, checkCodeError(checkCodeRouteGenerate, err)
	}
	mountURLFiles, err := project.GenerateMountURLHelperFiles(paths, manifest)
	if err != nil {
		return nil, checkCodeError(checkCodeURLGenerate, err)
	}
