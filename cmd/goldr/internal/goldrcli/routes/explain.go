...
func renderExplainOutput(writer io.Writer, routesDir string, explanation wiring.RouteExplanation, style ansi.Style) error {
	...
	if match.Declaration != nil {
		if err := renderExplainDeclarationOutput(writer, routesDir, match); err != nil {
			return err
		}
	}
...
func renderExplainDeclarationOutput(writer io.Writer, routesDir string, match wiring.RouteExplanationMatch) error {
	declaration := match.Declaration
	if _, err := fmt.Fprintln(writer); err != nil {
		return err
	}
	if _, err := fmt.Fprintln(writer, "DECLARATION"); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "kind", routeSurfaceDeclarationKindText(declaration)); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "source", routeSourceDisplayPath(routesDir, declaration.Source)); err != nil {
		return err
	}
	if declaration.Mount != nil {
		if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "owner", routeSourceDisplayPath(routesDir, declaration.Mount.Owner)); err != nil {
			return err
		}
	}
	if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "name", routeSurfaceDeclarationNameText(declaration)); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "title", routeSurfaceDeclarationTitleText(declaration)); err != nil {
		return err
	}
	...
	if declaration.Nav.Label != "" || declaration.Nav.Key != "" {
		if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "nav", routeSurfaceDeclarationNavText(declaration)); err != nil {
			return err
		}
	}
	if len(declaration.TrailKeys) > 0 {
		if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "trailkey", routeSurfaceDeclarationTrailKeysText(declaration)); err != nil {
			return err
		}
	}
	if declaration.Kit != nil {
		...
		if _, err := fmt.Fprintf(writer, "  %-8s %s\n", "new", declaration.Kit.New); err != nil {
			return err
		}
	}
	...
	if len(declaration.Destinations) > 0 {
		...
		for _, destination := range declaration.Destinations {
			...
			if destination.TrailKey != "" {
				if _, err := fmt.Fprintf(writer, " trail_key=%s", destination.TrailKey); err != nil {
					return err
				}
			}
			if _, err := fmt.Fprintln(writer); err != nil {
	...
	if len(declaration.InboundDestinations) > 0 {
		if _, err := fmt.Fprintln(writer); err != nil {
			return err
		}
		if _, err := fmt.Fprintln(writer, "INBOUND DESTINATIONS"); err != nil {
			return err
		}
		for _, destination := range declaration.InboundDestinations {
			if _, err := fmt.Fprintf(writer, "  %-18s %s -> %s trail_key=%s\n", destination.Name, destination.Helper, destination.Source, destination.TrailKey); err != nil {
				return err
			}
		}
	}
...
func renderExplainImplementationOutput(writer io.Writer, match wiring.RouteExplanationMatch) error {
	declaration := match.Declaration
	switch {
	case declaration.Page != nil:
		_, err := fmt.Fprintf(writer, "  %-8s %s -> %s\n", "page", declaration.Page.Handler, declaration.Page.Adapter)
		return err
	case declaration.Fragment != nil:
		if _, err := fmt.Fprintf(writer, "  %-8s %s -> %s\n", "fragment", declaration.Fragment.Name, match.Route); err != nil {
			return err
		}
		_, err := fmt.Fprintf(writer, "  %-8s %s -> %s\n", "handler", declaration.Fragment.Handler, declaration.Fragment.Adapter)
		return err
	case declaration.Action != nil:
		actionName := declaration.Action.Name
		if declaration.Action.Index {
			actionName = "index"
		}
		if _, err := fmt.Fprintf(writer, "  %-8s %s %s\n", "action", declaration.Action.Method, actionName); err != nil {
			return err
		}
		_, err := fmt.Fprintf(writer, "  %-8s %s -> %s\n", "handler", declaration.Action.Handler, declaration.Action.Adapter)
		return err
	default:
		return nil
	}
