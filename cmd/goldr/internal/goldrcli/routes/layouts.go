...
func renderLayoutMapNode(writer io.Writer, routesDir string, node *wiring.RouteLayoutMapNode, prefix string, last bool, style ansi.Style) error {
	...
	for _, page := range node.Pages {
		entryIndex++
		text := layoutMapRouteText("page", page.Methods, page.Route, page.Params, layoutMapSourceText(routesDir, page.Source, page.Owner), style)
	...
	for _, fragment := range node.Fragments {
		entryIndex++
		text := layoutMapRouteText("fragment (not wrapped)", fragment.Methods, fragment.Route, fragment.Params, layoutMapSourceText(routesDir, fragment.Source, fragment.Owner), style)
		if err := renderLayoutMapLeaf(writer, childPrefix, text, entryIndex == entryCount); err != nil {
			return err
		}
	}
...
func layoutMapActionText(routesDir string, action wiring.RouteLayoutMapAction, style ansi.Style) string {
	source := fmt.Sprintf("%s (%s)", layoutMapSourceText(routesDir, action.Source, action.Owner), action.Function)
	return layoutMapRouteText("action (layout-aware)", action.Methods, action.Route, action.Params, source, style)
}

func layoutMapSourceText(routesDir string, source string, owner string) string {
	text := routeSourceDisplayPath(routesDir, source)
	if owner != "" {
		text += "  owner: " + owner
	}
	return text
