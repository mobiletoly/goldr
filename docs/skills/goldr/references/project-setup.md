...
When the Goldr app root is nested inside a larger repository, pass the app root
explicitly:

```bash
go tool goldr generate --app-root <app-root>
...
Shared implementation packages can live outside `app/routes`, for example
`app/reports`, when multiple route directories bind them with
`goldr.KitRouteDef[K]`. Those packages do not own URLs; the filesystem route
