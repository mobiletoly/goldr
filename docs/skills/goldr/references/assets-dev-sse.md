...
`assets clean` removes stale Goldr-managed files that Goldr can prove it owns
from `assets/dist`. It is fail-closed: it uses `assets/.goldr/assets.json` and
does not delete arbitrary files from `assets/dist`.
