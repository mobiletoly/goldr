...
test("Svelte island saves, unmounts for boosted navigation, and remounts", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "About the Svelte island" })).toBeVisible();
  ...
  await page.goBack();
  await expect(page.getByLabel("Project name")).toHaveValue("Calm Svelte editor");
